...
func (db *DB) UpdateOne(collection string, filter interface{}, object interface{}) error {
	col := db.database.Collection(collection)
	_, err := col.UpdateOne(db.ctx, filter, object)
	return err
}
...
func (db *DB) DeleteMany(collection string, filter interface{}) error {
	col := db.database.Collection(collection)
	_, err := col.DeleteMany(db.ctx, filter)
	return err
}
